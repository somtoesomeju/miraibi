...
<div style={{
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
    <div style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(29,158,117,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Sparkles size={14} color="#1D9E75" />
    </div>
    <div>
      <div style={{ fontSize: 14, fontWeight: 500, color: '#f0ede8' }}>Mirai analysis</div>
      <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: '#666', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 2 }}>Generated by Claude Sonnet</div>
    </div>
  </div>
  <div style={{ fontSize: 14, color: '#c4c4c4', lineHeight: 1.7 }}>
    <ReactMarkdown
      components={{
        h2: ({children}) => <div style={{ color: '#f0ede8', fontWeight: 600, fontSize: 13, marginBottom: 10, marginTop: 20, fontFamily: 'Syne, sans-serif', letterSpacing: '-0.1px' }}>{children}</div>,
        h3: ({children}) => <div style={{ color: '#f0ede8', fontWeight: 600, fontSize: 12, marginBottom: 6, marginTop: 14 }}>{children}</div>,
        strong: ({children}) => <span style={{ color: '#f0ede8', fontWeight: 600 }}>{children}</span>,
        p: ({children}) => <p style={{ margin: '6px 0', color: '#c4c4c4', fontSize: 14, lineHeight: 1.7 }}>{children}</p>,
        ul: ({children}) => <ul style={{ paddingLeft: 0, margin: '8px 0', color: '#c4c4c4', listStyle: 'none' }}>{children}</ul>,
        li: ({children}) => <li style={{ margin: '8px 0', fontSize: 14, paddingLeft: 16, position: 'relative' as const, lineHeight: 1.6 }}>
          <span style={{ position: 'absolute' as const, left: 0, top: 10, width: 4, height: 4, borderRadius: 4, background: '#1D9E75' }} />
          {children}
        </li>,
        table: ({children}) => <table style={{ width: '100%', borderCollapse: 'collapse' as const, margin: '12px 0', fontSize: 13, fontFamily: 'DM Mono, monospace' }}>{children}</table>,
        th: ({children}) => <th style={{ textAlign: 'left' as const, padding: '8px 12px', borderBottom: '0.5px solid #2a2a2a', color: '#888', fontWeight: 500 }}>{children}</th>,
        td: ({children}) => <td style={{ padding: '8px 12px', borderBottom: '0.5px solid #1f1f1f', color: '#c4c4c4' }}>{children}</td>,
      }}
    >
      {insight.insight}
    </ReactMarkdown>
